...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card>currentFirst=firstSorcererCards.getFirst();

        SLLNode<Card> tmp=new  SLLNode<Card>(null,null);

        int best=currentFirst.element.significance();
         while (currentFirst!=null){
            if( currentFirst.element.significance()>best){

                best=currentFirst.element.significance();
                tmp=currentFirst;
        ...
        Card bestcard =new Card(tmp.element.getType(),tmp.element.getHealth(),tmp.element.getMagicPower());

        firstSorcererCards.delete(tmp);


        SLLNode<Card>currentSecond = secondSorcererCards.getFirst();
        int count=1;
         while (currentSecond!=null){
            if( count==4){
                secondSorcererCards.insertAfter(bestcard,currentSecond);
                break;
            }
            count++;
